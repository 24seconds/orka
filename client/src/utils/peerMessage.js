...
import { FINGERPRINT_SIZE, HEADER_SIZE_IN_BYTES } from "../constants/constant";
  ...
  const parsedMessage = ((rawMessage) => {
    try {
      ...
      if (!(message["messageType"] in PEER_MESSAGE_TYPE)) {
        throw new Error(
          `This peerMessageType is not supported! type: ${message["messageType"]}`
        );
      }
...
async function concatHeaderAndChunk(fingerprint, chunk) {
  ...
  const chunkWithHeader = await new Blob([
...
function parseChunkAndHeader(chunkWithHeader) {
  ...
  const fingerprint = new TextDecoder().decode(
    uInt8Array.slice(0, FINGERPRINT_SIZE)
  );
